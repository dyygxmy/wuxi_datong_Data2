...
//======================================================
//从串口中读取数据
int SerialCom::read_datas_tty(int fd, char *rcv_buf, int TimeOut, int Len)
{
    int retval;
    fd_set rfds;
    struct timeval tv;
...
//向串口传数据
int SerialCom::send_data_tty(int fd, char *send_buf, int Len)
{
    ssize_t ret;
...
void SerialCom::close_fd(int fd)
{
   close(fd);
}
