...
void FisUpdate::fisupdateFunc()
{
    if(WIFI_STATE)
    {
        ...
        if(db1.isOpen() && db2.isOpen() && QSqlDatabase::contains("SQLServerconnection") && QSqlDatabase::contains("mysqlconnection"))
        {
            ...
            for(int i=0;i<6;i++)
                query2.next();
            QString condition = "RecordID, Werk, SPJ, KNR, VIN ";
...
//            update_name = update_name+"=?";
            ...
            QString aff= "SELECT "+condition+" FROM Data.FisPreview WHERE RecordId >"+MysqlMax+" and Line_ID ="+QString::number(Line_ID);
            query1.exec(aff);
            while(query1.next())
            {
                //qDebug()<<"%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%";
